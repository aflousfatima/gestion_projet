...
async def get_token(authorization: Optional[str] = Header(None)) -> Optional[str]:
    ...
    logger.debug(f"Extracted token: {token[:10]}...")  # Log only first 10 chars for security
    return token

def validate_token(token: str) -> str:
    logger.debug(f"Validating token with AUTH_SERVICE_URL: {settings.AUTH_SERVICE_URL}")
    try:
...
async def websocket_chat(websocket: WebSocket):
    ...
    try:
        # Wait for initial message with token, with a timeout (10 seconds)
        ...
        while True:
            ...
            # Use websocket.receive_json() without timeout for non-blocking wait
