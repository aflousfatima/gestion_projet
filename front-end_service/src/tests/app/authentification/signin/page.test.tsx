...
jest.mock('next/link', () => {
  const MockedLink = ({ children, href, ...props }) => <a href={href} {...props}>{children}</a>;
...
describe('SigninPage Component', () => {
  ...
  it('renders the SigninPage form correctly', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('heading', { name: /Welcome Back to AGILIA/i })).toBeInTheDocument();
      ...
      expect(screen.getByRole('link', { name: /Sign Up/i })).toHaveAttribute('href', '/authentification/signup');
      expect(screen.getByRole('link', { name: /Forgot your password ?/i })).toHaveAttribute('href', '/authentification/signup');
  ...
  it('updates form inputs correctly', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByPlaceholderText('e-mail')).toHaveValue('john@example.com');
  ...
  it('handles form submission error for invalid credentials', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Error trying to connect. Please Retry./i)).toBeInTheDocument();
    });
  });
